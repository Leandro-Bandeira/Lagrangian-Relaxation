...
bool not_violation(std::vector<double>* weight_restr){
	int sumWeight = 0;
...
void getMatrizAdj(std::vector < std::vector<int>>* matrizAdj, std::vector < std::pair<int,int>>* edges){

	int qVertices = matrizAdj->size();

	for(int i = 0; i < qVertices; i++){
		for(std::pair<int,int> aresta: *edges){
			matrizAdj->at(aresta.first)[aresta.second] = matrizAdj->at(aresta.second)[aresta.first] = 1;
		}
	}
}
